...
export default {
  ...
  methods: {
    ...
    //  const dataUser = await axios.get(`${baseURL}/user?email=${this.user.email}&password=${this.user.password}`)  ;
     const dataUser = await axios.get(`http://localhost:3001/user?email=${this.user.email}&password=${this.user.password}`)  ;
    //  const empData = await axios.get(`${baseURL}/employee`)  ;
