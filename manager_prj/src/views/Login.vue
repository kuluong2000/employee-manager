<template>
  <div class="d-flex justify-center align-center" style="height: 100%">
                ...
                <v-row>
                  <v-col cols="12" md="6">
                    <v-card-text class="mt-12">
                      ...
                      <v-row align="center" justify="center" v-on:keyup.enter="login">
                        <v-col cols="12" sm="8">
                          <v-text-field label="Email" outlined dense color="blue" autocomplete="false" class="mt-12 mb-1" v-model="user.email" :rules="[requiredEmail]" />
                          <!-- <h5 v-if="user.email !== 'sydung@gmail.com'" style="color: red;margin: -20px 0 20px">Vui lòng nhập đúng cú pháp</h5> -->
                          <v-text-field label="Password" outlined dense color="blue" autocomplete="false" type="password" v-model="user.password" :rules="[requiredPassword]" />
                    ...
                    <v-card-text class="mt-12">
                      ...
                      <v-row align="center" justify="center" v-on:keyup.enter="signup">
                        <v-col cols="12" sm="8">
                          <v-row>
                            <v-col cols="12" sm="6">
                              <v-text-field label="First Name" outlined dense color="blue" autocomplete="false" class="mt-4" v-model="userSignUp.firstName" />
                            </v-col>
                            <v-col cols="12" sm="6">
                              <v-text-field label="Last Name" outlined dense color="blue" autocomplete="false" class="mt-4" v-model="userSignUp.lastName" />
                            </v-col>
                          </v-row>
                          <v-text-field label="Email" outlined dense color="blue" autocomplete="false" v-model="userSignUp.email" />
                          <v-text-field label="Password" outlined dense color="blue" autocomplete="false" type="password" v-model="userSignUp.password" />
                          <v-text-field label="Confirm Password" outlined dense color="blue" autocomplete="false" type="password" v-model="userSignUp.passwordConfirm" />
                          <v-row>
                            <v-col cols="12" sm="7">
    ...
    <popup :show="showDialog" :cancel="cancel" :confirm="confirm" title="Thông báo?" text="Ok! Mình sẽ kiểm tra lại" description="Email hoặc mật khẩu không đúng!!!"></popup>
    <popup :show="showDialogSignUp" :cancel="cancel" :confirm="confirm" title="Thông báo?" text="Ok! Mình sẽ kiểm tra lại" description="Vui lòng điền đầy đủ thông tin !!!"></popup>
    <popup :show="showDialogPassword" :cancel="cancel" :confirm="confirm" title="Thông báo?" text="Ok! Mình sẽ kiểm tra lại" description="Kiểm tra lại mật khẩu vừa nhập !!!"></popup>
    <popup :show="showDialogSuccess" :cancel="cancel" :confirm="confirm" title="Thông báo?" text="OKE ^^" description="Đăng ký thành công !!!"></popup>
  </div>
</template>
...
export default {
  ...
  methods: {
    async login() {
      ...
      const resPass = [...datalocal].find((el) => el.password === this.user.password);
      if (resEmail && resPass) {
